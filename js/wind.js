...
const WindEngine = (() => {
  ...
  /**
   * KOM-Wahrscheinlichkeits-Score (0–100%)
   * Kombiniert: Rückenwind-Stärke, KOM-Rückstand, Gradient, Windvarianz
   *
   * @param {number} effectiveTailwind  - km/h (aus scoreSegment)
   * @param {number} komGapSeconds      - Rückstand auf KOM in Sekunden (0 = du bist KOM)
   * @param {number} segmentDistanceKm  - Segmentlänge in km
   * @param {number} gradientPct        - Steigung
   * @param {number} windVarianzKmh     - Standardabweichung des Winds (optional)
   */
  function komProbability(effectiveTailwind, komGapSeconds = null, segmentDistanceKm = 1, gradientPct = 0, windVarianzKmh = 0) {
    // Base score from tailwind (0–60 Punkte)
    let score = 0;
    if (effectiveTailwind >= 15)      score = 60;
    else if (effectiveTailwind >= 8)  score = 45 + (effectiveTailwind - 8) / 7 * 15;
    else if (effectiveTailwind >= 3)  score = 28 + (effectiveTailwind - 3) / 5 * 17;
    else if (effectiveTailwind >= -3) score = 15 + (effectiveTailwind + 3) / 6 * 13;
    else if (effectiveTailwind >= -8) score = 5  + (effectiveTailwind + 8) / 5 * 10;
    else                              score = Math.max(0, 5 + effectiveTailwind);

    // KOM-Rückstand-Bonus (0–30 Punkte)
    // Schätzung: wie viel Sekunden bringt der Rückenwind?
    // Faustregel: +1 km/h Rückenwind ≈ -1.5s/km bei 35km/h
    if (komGapSeconds !== null && komGapSeconds >= 0) {
      const typicalSpeed = gradientPct < 3 ? 35 : gradientPct < 6 ? 22 : 15;
      const tailwindGainSec = (effectiveTailwind * 1.5 * segmentDistanceKm);
      if (komGapSeconds === 0) {
        score += 30; // Already KOM
      } else {
        const closureRatio = Math.min(1, tailwindGainSec / komGapSeconds);
        score += closureRatio * 30;
      }
    } else {
      score += 10; // No gap data — neutral bonus
    }

    // Gradient-Penalty: bei steilen Anstiegen hilft Wind wenig
    const relevance = windRelevanceFactor(gradientPct);
    if (relevance < 0.5) score *= 0.7;

    // Windvarianz-Penalty: stark schwankender Wind = unzuverlässig
    if (windVarianzKmh > 8)  score *= 0.85;
    if (windVarianzKmh > 15) score *= 0.7;

    return Math.round(Math.max(0, Math.min(100, score)));
  }

  /**
   * Beide Fahrtrichtungen eines Segments bewerten.
   * Gibt zurück welche Richtung besser ist + effektive Tailwinds.
   *
   * @param {number} bearing         - normaler Bearing (Start→Ziel)
   * @param {object} weather         - { windDirection, windSpeed }
   * @param {number} gradientPct     - Steigung (für Vorwärtsrichtung)
   */
  function bothDirections(bearing, weather, gradientPct = 0) {
    const reverseBearing = (bearing + 180) % 360;
    const reverseGradient = -gradientPct; // Downhill in reverse

    const forward = scoreSegment(
      { bearing, gradient: gradientPct, typicalSpeedKmh: gradeToSpeed(gradientPct) },
      weather
    );
    const reverse = scoreSegment(
      { bearing: reverseBearing, gradient: reverseGradient, typicalSpeedKmh: gradeToSpeed(reverseGradient) },
      weather
    );

    const betterIsForward = forward.effectiveTailwind >= reverse.effectiveTailwind;
    return {
      forward,
      reverse,
      recommended: betterIsForward ? 'forward' : 'reverse',
      gain: Math.abs(forward.effectiveTailwind - reverse.effectiveTailwind),
      forwardBearing: bearing,
      reverseBearing,
    };
  }

  function gradeToSpeed(g) {
    const abs = Math.abs(g);
    if (abs < 1) return 38; if (abs < 3) return 32;
    if (abs < 6) return 22; if (abs < 10) return 15;
    return 11;
  }

  /**
   * Geografisches Clustering von Segmenten.
   * Einfaches Grid-Clustering mit konfigurierbarem Radius.
   *
   * @param {Array}  segments  - Array mit { id, name, lat, lon, ... }
   * @param {number} radiusKm  - Cluster-Radius in km
   */
  function clusterSegments(segments, radiusKm = 15) {
    const clusters = [];
    const assigned = new Set();

    segments.forEach(seg => {
      if (assigned.has(seg.id)) return;

      const cluster = { segments: [seg], lat: seg.lat, lon: seg.lon };
      assigned.add(seg.id);

      segments.forEach(other => {
        if (assigned.has(other.id)) return;
        const dist = haversineKm(seg.lat, seg.lon, other.lat, other.lon);
        if (dist <= radiusKm) {
          cluster.segments.push(other);
          assigned.add(other.id);
        }
      });

      // Cluster center = average coords
      cluster.lat = cluster.segments.reduce((s, x) => s + x.lat, 0) / cluster.segments.length;
      cluster.lon = cluster.segments.reduce((s, x) => s + x.lon, 0) / cluster.segments.length;
  ...
  /**
   * Haversine-Distanz in km zwischen zwei GPS-Punkten
   */
  ...
  /**
   * Beste Fahrtrichtung als lesbarer String
   * z.B. "↗ NW → SO" oder "Von West nach Ost"
   */
  function directionLabel(bearing) {
    const dirs = ['N','NO','O','SO','S','SW','W','NW'];
    const from = dirs[Math.round(((bearing + 180) % 360) / 45) % 8];
    const to   = dirs[Math.round(bearing / 45) % 8];
    return `${from} → ${to}`;
  }

  /**
   * Windvarianz aus stündlichen Forecast-Daten schätzen
   */
  function windVariance(forecast) {
    if (!forecast?.length) return 0;
    const speeds = forecast.slice(0, 6).map(f => f.windSpeed);
